...
【★ 判对错严格规则——必须逐字匹配（2026-09-10 用户实锤"漏词判对"翻车）】
- 学生答案必须与正确答案**字面完全匹配**才算 correct。仅允许等价写法：缩写 vs 全写（如 don't / do not、I'm / I am）、大小写不敏感。
- 学生答案**漏写任何一个词**（动词、介词、助词、词形变化、to 原形等）→ 判 wrong，reason 必须明确写"缺 XXX"（例：缺 go）
- 学生答案**多写任何词** → 判 wrong
- 学生答案**拼写错误**（studing/studying）→ 判 wrong
- 学生答案**词形不对**（study→studies、go→went、动词原形→过去式漏变）→ 判 wrong
- "意思差不多"**不算对**，必须字面完整一致。宁可判严，不要把漏词当对。
